...
import {
  GET_SEARCH_LIST_NOT_REG,
  GET_USER_LIST_CONFIRMED,
  GET_USER_LIST_NOTCONFIRMED,
...
export const userLogin = (values, formikLogin) => {
  return async (dispatch) => {
    try {
      let resultLogin = await http.post(
...
export const userCancelCourse = (maKhoaHoc) => {
  return async (dispatch) => {
    ...
    if (credentailLocal) {
      ...
      try {
        let resultCancel = await http.post(
  ...
  return async (dispatch) => {
    try {
      let result = await http.get(
        `/api/QuanLyNguoiDung/TimKiemNguoiDung?tuKhoa=${v}`
      );
